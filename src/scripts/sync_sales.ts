...
async function syncSales() {
    ...
    try {
        ...
        // Buscar todos os pedidos entregues que NÃO têm transação financeira
        const query = `
      SELECT o.* 
      FROM orders o
    ...
    `;
        ...
        // Buscar ou criar conta e categoria padrão
        let accountId;
        ...
        for (const order of result.rows) {
          ...
          transaction_date, due_date, payment_date, status, user_id
                ...
                [
                    ...
                    order.order_date, // Usar a data do pedido
                    order.order_date, // Usar a data do pedido como vencimento inicial
                    paymentDate,
                    transactionStatus,
                    order.user_id || null
                ]
            ...
            // Atualizar saldo APENAS se estiver pago
            if (isPaid) {
                await client.query(
