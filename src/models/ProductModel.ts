...
export class ProductModel extends BaseModel {
  ...
  async findActive(): Promise<ApiResponse<Product[]>> {
    const query = `
      SELECT 
        p.*,
        COALESCE(
  ...
  async findWithStock(): Promise<ApiResponse> {
    try {
      const query = `
        SELECT 
          p.*,
  ...
  async findByPriceRange(minPrice: number, maxPrice: number): Promise<ApiResponse<Product[]>> {
    return await this.customQuery(
      'SELECT * FROM products WHERE price_sell BETWEEN $1 AND $2 AND status = $3 ORDER BY price_sell',
      [minPrice, maxPrice, 'Ativo']
    );
  }

  // Produtos mais vendidos
  async getMostSold(limit: number = 10): Promise<ApiResponse> {
    try {
      const query = `
        SELECT 
          p.*,
  ...
  async getLowStock(): Promise<ApiResponse> {
    try {
      const query = `
        SELECT 
          p.*,
  ...
  async getProfitabilityReport(): Promise<ApiResponse> {
    try {
      const query = `
        SELECT 
          p.*,
  ...
  // Histórico de preços (se implementarmos controle de versão de preços)
  async getPriceHistory(productId: number): Promise<ApiResponse> {
    try {
      // Por enquanto, retorna apenas o preço atual
      // Futuramente pode ser implementado um histórico completo
      const query = `
        SELECT 
          id,
          name,
