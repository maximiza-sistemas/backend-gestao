...
class RouteModel extends BaseModel_1.BaseModel {
    constructor() {
        super('delivery_routes');
    }
    async findAllWithDetails(options = {}) {
        try {
            const { page = 1, limit = 50, sort = 'route_date', order = 'DESC', search, status, date_from, date_to } = options;
    ...
    async createWithStops(routeData, stops, userId) {
        ...
        `;
                ...
                if (stops && stops.length > 0) {
                    for (let i = 0; i < stops.length; i++) {
                        const stop = stops[i];
                        await client.query(`INSERT INTO delivery_route_stops (
