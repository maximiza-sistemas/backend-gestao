...
class StockModel extends BaseModel_1.BaseModel {
    constructor() {
        super('stock');
    }
    async findAllWithDetails(options = {}) {
        try {
            const { page = 1, limit = 50, sort = 'p.name', order = 'ASC', search, location_id, product_id, low_stock_only = false } = options;
    ...
    async updateStock(productId, locationId, stockData, userId) {
        return await this.executeTransaction(async (client) => {
            try {
                const currentStockResult = await client.query('SELECT * FROM stock WHERE product_id = $1 AND location_id = $2', [productId, locationId]);
          ...
          `;
                    ...
                    if (userId) {
                        ...
                        if (changes.length > 0) {
                            await client.query('INSERT INTO activity_logs (user_id, action, table_name, record_id) VALUES ($1, $2, $3, $4)', [userId, `Atualizou estoque: ${changes.join(', ')}`, 'stock', result.rows[0].id]);
                        }
                    }
                    return result.rows[0];
                }
    ...
    async getMovements(options = {}) {
        try {
            const { page = 1, limit = 50, sort = 'created_at', order = 'DESC', product_id, location_id, movement_type, date_from, date_to } = options;
            let whereClause = 'WHERE 1=1';
            const params = [];
            let paramIndex = 1;
            if (product_id) {
    ...
    async deleteStock(productId, locationId) {
        try {
            const result = await this.customQuery(`DELETE FROM stock WHERE product_id = $1 AND location_id = $2 RETURNING *`, [productId, locationId]);
