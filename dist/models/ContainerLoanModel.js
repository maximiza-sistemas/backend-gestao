...
exports.ContainerLoanModel = void 0;
const BaseModel_1 = require("./BaseModel");
const database_1 = require("../config/database");
class ContainerLoanModel extends BaseModel_1.BaseModel {
    constructor() {
        super('container_loans');
    }
    async findAllWithDetails(options = {}) {
        try {
            const { page = 1, limit = 50, sort = 'loan_date', order = 'DESC', status, direction, entity_type, search, location_id } = options;
    ...
    async returnLoan(id, actualReturnDate) {
        try {
            const result = await (0, database_1.query)(`UPDATE container_loans 
         SET status = 'Devolvido', 
             actual_return_date = $2,
             updated_at = CURRENT_TIMESTAMP
         WHERE id = $1 AND status = 'Ativo'
         RETURNING *`, [id, actualReturnDate || new Date().toISOString().split('T')[0]]);
    ...
    async cancelLoan(id) {
        try {
            const checkResult = await (0, database_1.query)(`SELECT * FROM container_loans WHERE id = $1`, [id]);
            ...
            if (loan.direction === 'Saída') {
                await (0, database_1.query)(`UPDATE stock 
           SET empty_quantity = empty_quantity + $1
           WHERE product_id = $2 AND location_id = $3`, [loan.quantity, loan.product_id, loan.location_id]);
            }
...
exports.ContainerLoanModel = ContainerLoanModel;
//# sourceMappingURL=ContainerLoanModel.js.map