...
const syncStock = async () => {
    ...
    try {
        ...
        const productsResult = await client.query('SELECT id, name FROM products WHERE status = $1', ['Ativo']);
        console.log('📦 Produtos ativos encontrados:', productsResult.rows.length);
        let locationsResult = await client.query('SELECT id, name FROM locations WHERE status = $1', ['Ativo']);
        if (locationsResult.rows.length === 0) {
            const newLocation = await client.query(`INSERT INTO locations (name, status) VALUES ($1, $2) RETURNING id, name`, ['Matriz', 'Ativo']);
            locationsResult.rows.push(newLocation.rows[0]);
            console.log('📍 Localização Matriz criada');
        }
        console.log('📍 Localizações ativas:', locationsResult.rows.map((l) => l.name).join(', '));
        let syncedCount = 0;
        for (const product of productsResult.rows) {
            for (const location of locationsResult.rows) {
                const insertResult = await client.query(`INSERT INTO stock (product_id, location_id, full_quantity, empty_quantity, maintenance_quantity, min_stock_level, max_stock_level)
...
syncStock();
//# sourceMappingURL=sync_stock.js.map